...
const Index = () => {
  ...
  const sendBtn=()=>{
    if (smsBody&&number) {
      ...
      axios.post("http://localhost:5000/api/v1/send-sms",details).then(res=>{
         console.log(res)
      }).catch(err=>{
            ...
            <div className='p-3'>
                  <div>
                    ...
                    <label>Bulk-SMS Body</label>
                        <textarea value={smsBody}  onChange={(e) => setB(e.target.value)} rows={3} style={{resize:"none"}} className='form-control'>
                        </textarea>
                    </div>
                    <div className='my-3'>
